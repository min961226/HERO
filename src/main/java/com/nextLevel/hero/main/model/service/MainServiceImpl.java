...
public class MainServiceImpl implements MainService{
	...
	public List<WorkAttitudeDTO> selectAllWorkAttitudeList(int companyNo, int idNo, Map searchMap) {
		...
		/* 사원수 검색 */
		int totalCount = mainMapper.selectMemberCount(searchMap, companyNo, idNo);
		...
		/* 사원 조회 */
		List<WorkAttitudeDTO> memberList = mainMapper.selectMemberList(searchMap, companyNo, idNo, selectCriteria );
		...
		for(int w = 0; w < memberList.size(); w++) {
			...
			/* 출퇴근 기록 조회 */
			List<WorkCommuteDTO> workSpList = mainMapper.selectWorkAttitudeList(memberInfo, formatedNow, idNo);
